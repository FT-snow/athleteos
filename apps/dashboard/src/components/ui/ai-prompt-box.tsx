  ...
  return (
    ...
    >
      <div className="mb-3 flex items-center gap-2">
        ...
        <span className="font-ui-mono text-sm text-white/80">
          {formatTime(time)}
        </span>
      </div>
      ...
      <TooltipProvider>
        ...
        >
          <div
            ref={ref}
            className={cn(
              // FIX: uniform px-3 pt-3 pb-2 padding on the outer wrapper
              // so the textarea, file previews, and action bar all share the same horizontal inset
              "rounded-3xl border border-[#444444] bg-[#1F2023] px-3 pt-3 pb-2 shadow-[0_8px_30px_rgba(0,0,0,0.24)] transition-all duration-300",
...
> = ({
  ...
  return (
    <Textarea
      ...
      // FIX: remove horizontal padding from textarea — the outer wrapper now owns
      // the horizontal inset, so px-0 keeps text flush with action buttons below
...
const PromptInputActions: React.FC<PromptInputActionsProps> = ({
  ...
  // FIX: replace p-0 pt-2 with just pt-1 — bottom spacing now comes from
  // the outer wrapper's pb-2, so no extra bottom padding needed here
    ...
    return (
      <>
        <PromptInput
          ...
          onDrop={handleDrop}
        >
          {/* FIX: file previews — removed p-0, use gap-2 mb-2 so thumbnails
              sit above the textarea with consistent spacing */}
